...
export default function SchedulerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await schedulerAuthAPI.login({ email, password });
      localStorage.setItem("token", res.token);

      // Store scheduler info for navbar
      if (res.user) {
        localStorage.setItem("schedulerName", res.user.name);
        localStorage.setItem("schedulerEmail", res.user.email);
      }
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      await schedulerAuthAPI.changePassword({
        currentPassword,
        newPassword,
      });

      // Password changed, redirect to dashboard
      navigate("/scheduler/dashboard");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-green-50 p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        ></div>
        <div
          className="absolute -bottom-8 left-1/2 w-72 h-72 bg-blue-300 rounded-full mix-blend-multiply filter blur-xl opacity-30"
      ...
      <div className="relative w-full max-w-md z-10">
        ...
        <div className="text-center mb-8">
          <div className="inline-block mb-4 transform hover:scale-105 transition-transform">
            <img src={Logo} alt="BusPulse logo" className="w-48" />
          </div>
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {mustChangePassword ? "Set New Password" : "Scheduler Login"}
          </h1>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          Scheduler access • BusPulse Bus Tracking System
        </p>
      </div>
